<div class="page-content page-anim" id="page-mata-pelajaran" style="display:none">
    ...
    <div style="margin-bottom:20px">
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px">
            ...
            {{-- Total Mapel --}}
            <div class="mapel-summary-card" style="background:#fff;border:1px solid #e2e8f0;border-radius:var(--radius);padding:18px 20px;display:flex;align-items:center;gap:12px;box-shadow:var(--shadow)">
            ...
            {{-- Kelompok A --}}
            @php
                $kelompokACount = $allMapel->where('kelompok', 'A')->count();
                $kelompokBCount = $allMapel->where('kelompok', 'B')->count();
                $kelompokCCount = $allMapel->where('kelompok', 'C')->count();
            @endphp
            <div class="mapel-summary-card" style="background:#fff;border:1px solid #e2e8f0;border-radius:var(--radius);padding:18px 20px;display:flex;align-items:center;gap:12px;box-shadow:var(--shadow)">
                ...
                <tr class="mapel-row"
                    ...
                    <td style="text-align:center">
                        <div style="display:inline-flex;align-items:center;gap:6px">
                            {{-- Edit Button --}}
                            <button onclick="editMapelModal({{ $m->id_mapel }}, '{{ htmlspecialchars($m->kode_mapel ?? '') }}', '{{ htmlspecialchars($m->nama_mapel) }}', '{{ htmlspecialchars($m->kelompok ?? '') }}')"
                            ...
                            {{-- Hapus Button --}}
                            <button onclick="hapusMapel({{ $m->id_mapel }}, '{{ htmlspecialchars($m->nama_mapel) }}', {{ (int) $m->jadwal_count }})"
...
function editMapelModal(id, kode, nama, kelompok){
    Swal.fire({
        ...
        html: `
            <div class="swal-form-container">
                ...
                <div class="swal-form-group">
                    <label for="swal-edit-mkelompok">Kelompok</label>
                    <select id="swal-edit-mkelompok" class="swal-form-input">
                        <option value="">-- Pilih Kelompok --</option>
                        <option value="A" ${kelompok === 'A' ? 'selected' : ''}>Kelompok A</option>
                        <option value="B" ${kelompok === 'B' ? 'selected' : ''}>Kelompok B</option>
                        <option value="C" ${kelompok === 'C' ? 'selected' : ''}>Kelompok C</option>
                    </select>
                </div>
            </div>
        `,
        ...
        preConfirm: () => {
            ...
            return {
                ...
                nama_mapel: newNama,
                kelompok: document.getElementById('swal-edit-mkelompok').value
            };
        }
    }).then(result => {
