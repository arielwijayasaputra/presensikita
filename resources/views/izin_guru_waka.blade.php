...
<title>Persetujuan Waka - PresensiKita</title>
...
<p class="intro">Halaman khusus Wakil Kepala Sekolah. Tidak perlu login untuk memberikan keputusan.</p>
<section class="info">
...
<div class="decision">
<h2>Keputusan Waka</h2>
<div class="row"><span class="label">Status</span><span class="badge badge-warning {{ $izin->status_waka === 'disetujui' ? 'badge-success' : ($izin->status_waka === 'ditolak' ? 'badge-danger' : '') }}">{{ ucfirst($izin->status_waka) }}</span></div>
...
<form method="POST" action="{{ $approvalUrl }}" onsubmit="return konfirmasiKeputusan(event, 'Waka')">@csrf
...
@else<p class="intro">Keputusan Waka sudah diberikan.</p>@endif
</div>
</main>
<script>
